...
current_image = cv2.imread(image_poses[posNo])

if current_image.shape[0] != 480 or current_image.shape[1] != 640:
  current_image = cv2.resize(current_image, (640, 480))


est = TfPoseEstimator(get_graph_path('mobilenet_thin'), target_size=(640, 480))
humans = est.inference(current_image, resize_to_default=False)

posed_image = TfPoseEstimator.draw_humans(current_image, humans, imgcopy=False)
